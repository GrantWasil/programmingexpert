...
class Deck:
    def __init__(self, cards=[]):
        ...
        valid_suits = ['H', 'D', 'C', 'S']
        if len(cards) == 0:
            self.cards = [f'{value}{suit}' for suit in valid_suits for value in valid_values]
        else:
            self.cards = cards
    ...
    def deal(self, n):
        if n > len(self.cards):
            return self.cards
        delt_cards = self.cards[-n:]
        self.cards = self.cards[:-n]
        return delt_cards
    
    def sort_by_suit(self):
        sort_order  = {'H': 0, 'D': 1, 'C': 2, 'S': 3}
        def get_suit_value(card):
            return sort_order[card[-1]]
        
        self.cards.sort(key=get_suit_value)
